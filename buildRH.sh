...
dnf upgrade -y
#&& dnf install -y epel-release 'dnf-command(config-manager)' && dnf config-manager --set-enabled crb
...
dnf install -y \
autoconf automake bison libtool yasm tcl meson ninja-build patch perl-FindBin alsa-lib-devel pulseaudio-libs-devel ncurses-devel diffutils python3-mako flex
